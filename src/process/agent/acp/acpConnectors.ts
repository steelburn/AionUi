...
async function prepareClaude(): Promise<BunPrepareResult> {
...
/**
 * Connect to a bun-based ACP backend with 2-phase retry.
 * Phase 1: attempt spawn + setup. If it fails, cleanup and warn.
 * Phase 2: retry with the same spawn configuration.
 */
async function connectBunBackend(config: {
  ...
  const { backend, bunPackage, prepareFn, workingDir, setup, cleanup } = config;
