...
/**
 * Register StarOffice endpoint handlers on the WsRouter.
 * Replaces initStarOfficeBridge() from src/process/bridge/starOfficeBridge.ts.
 */
export function registerStarOfficeHandlers(router: WsRouter): void {
  router.handle('star-office.detect-url', async ({ preferredUrl, force, timeoutMs }) => {
    try {
